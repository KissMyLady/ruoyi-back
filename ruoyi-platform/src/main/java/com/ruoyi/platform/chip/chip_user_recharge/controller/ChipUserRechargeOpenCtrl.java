...
import com.ruoyi.platform.chip.chip_user_recharge.service.impl.ChipUserRechargeOpenServiceImpl;
...
/**
 * @Author: KissMyLady
 * @Date: Created in 2023/9/26 17:08
 * @Description: 开放接口 用户充值
 */
@RestController
@RequestMapping("/openApi/chipUserRecharge/")
public class ChipUserRechargeOpenCtrl {

    @Autowired
    private ChipUserRechargeOpenServiceImpl chipUserRechargeOpenService;
    ...
    @PostMapping("/edit")
    public ResultVo<?> edit(@RequestBody ChipUserRecharge dto, HttpServletRequest request) {
        ...
        String title = hasCheckMd5 ? "api请求推送数据到用户充值表" : "api请求推送数据到用户充值表-[未使用md5校验接口安全性]";
        ...
        ResultVo<?> resultVo = chipUserRechargeOpenService.editData(dto);
    ...
    @PostMapping("/push")
    public ResultVo<?> pushData(@RequestBody ChipUserRecharge dto, HttpServletRequest request) {
        RequestApiKeySendLog logEntity = new RequestApiKeySendLog();
        long startTime = System.currentTimeMillis();
        String methodStr = request.getMethod();
        String userAgentStr = request.getHeader("user-agent");
        ...
        ResultVo<?> resultVo = chipUserRechargeOpenService.pushData(dto);
